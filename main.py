...
def analyze_body(data: FrameInput):
    ...
    if frame is None:
        reset_all_queues()
        return {"status": "error", "version": API_VERSION}

    mask = get_silhouette_mask(frame)
    if mask is None:
        reset_all_queues()
        return {"status": "scanning", "version": API_VERSION}
    
    mask = clean_mask_for_mode(mask, data.scan_mode)
    
    if not is_upright(mask, data.scan_mode):
        reset_all_queues()
        return {"status": "pose_not_upright", "version": API_VERSION}
    ...
    shoulder_w_q.append(shoulder_w)
    waist_w_q.append(waist_w)
    hip_w_q.append(hip_w)

    if len(waist_w_q) >= MIN_STABLE_FRAMES and statistics.pstdev(waist_w_q) > MAX_WAIST_STD:
        reset_all_queues()
        return {"status": "stabilizing", "version": API_VERSION}

    SR = shoulder_w / hip_w
    WR = waist_w / hip_w

    shoulder_hip_q.append(SR)
    waist_hip_q.append(WR)

    if len(shoulder_hip_q) < MIN_STABLE_FRAMES:
        return {"status": "stabilizing", "version": API_VERSION}

    if statistics.pstdev(shoulder_hip_q) > MAX_SR_STD:
        return {"status": "stabilizing", "version": API_VERSION}

    avg_SR = float(np.clip(statistics.mean(shoulder_hip_q), *SHOULDER_HIP_LIMITS))
    avg_WR = float(np.clip(statistics.mean(waist_hip_q), *WAIST_HIP_LIMITS))
    ...
    if statistics.pstdev(shoulder_hip_q) > 0.05: confidence_val -= 0.10

    confidence_val = float(np.clip(confidence_val, 0.0, 1.0))
    confidence_q.append(confidence_val)
    final_conf = round(statistics.mean(confidence_q), 2)

    if final_conf < CONFIDENCE_LOCK_THRESHOLD:
        reset_all_queues() # Reset queues on fail (Polish A)
        return {"status": "scan_not_reliable", "version": API_VERSION}
    ...
    return {
        "status": "locked",
        "version": API_VERSION,
        "scan_id": str(uuid4()), # Scan ID (Polish C)
...
async def analyze_skin(files: List[UploadFile] = File(...)):
    try:
        ...
        if avg_l > 75: tone = "fair"
        elif avg_l > 60: tone = "light-medium"
        elif avg_l > 45: tone = "medium"
        elif avg_l > 30: tone = "medium-deep"
        else: tone = "deep"

        undertone = "warm" if (avg_b - avg_a) > 8 else "cool" if (avg_b - avg_a) < -8 else "neutral"
        final_shape = "oval" # Fallback default for stability

        print(f"✅ DEBUG: SUCCESS! Tone: {tone}, Undertone: {undertone}")
